  ...
  <div v-if="sneakers.length > 0">
    <div class="space-y-6 mt-6">
      <div v-for="sneaker in sneakers" :key="sneaker.id" class="flex flex-col md:flex-row items-center justify-between bg-white rounded-lg p-6 shadow-sm">
        ...
        <div class="flex items-start cursor-pointer px-2.5 py-0.5 rounded" :class="{'bg-green-100': isUpgradable(sneaker)}" @click="sneakerBeingViewed = sneaker">
  ...
  <DialogModal :show="sneakerBeingViewed" @close="sneakerBeingViewed = false">
    ...
    <template #content>
      <ul class="mt-3 text-sm">
        ...
        <li>
          <span class="text-gray-600 mr-1">Daily Maintenance Cost</span> {{ numberFormat(sneakerBeingViewed.daily_expense_gmt) }} GMT
          <span class="text-gray-600">(incl. {{ numberFormat(sneakerBeingViewed.daily_repair_gst) }} GST repair cost)</span>
        </li>
      </ul>
      ...
      <ul class="mt-3 text-sm">
        ...
        <li>
          <span class="text-gray-600 mr-1">Daily Earning</span> {{ numberFormat(sneakerBeingViewed.daily_earn_gmt) }} GMT
        </li>
        <li>
          <span class="text-gray-600 mr-1">Daily Net Profit</span> {{ numberFormat(sneakerBeingViewed.daily_earn_gmt - sneakerBeingViewed.daily_expense_gmt) }}
          GMT
        </li>
      </ul>
      ...
      <ul class="mt-3 text-sm" v-if="sneakerBeingViewed.daily_roi < sneakerBeingViewed.daily_roi_max">
        <li>
          <span class="text-gray-600 mr-1">Potential APY</span> {{ numberFormat(sneakerBeingViewed.apy_max) }}%
        </li>
        <li>
          <span class="text-gray-600 mr-1">Payback Period</span> {{ sneakerBeingViewed.payback_period_max }}
        </li>
        <li>
          <span class="text-gray-600 mr-1">Daily Earning</span> {{ numberFormat(sneakerBeingViewed.daily_earn_max_gmt) }} GMT
        </li>
        <li>
          <span class="text-gray-600 mr-1">Daily Net Profit</span> {{ numberFormat(sneakerBeingViewed.daily_earn_max_gmt - sneakerBeingViewed.daily_expense_gmt) }}
          GMT
        </li>
      </ul>
