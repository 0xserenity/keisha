    ...
    <div v-if="sneakers.length > 0">
        <div class="space-y-6 mt-6">
            <div v-for="sneaker in sneakers" :key="sneaker.id"
                 class="flex flex-col md:flex-row items-center justify-between bg-white rounded-lg p-6 shadow-sm">
          ...
          <span v-if="sneaker.comfort_socket > 0"
          ...
          <span class="bg-red-100 text-red-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded">
            ...
            {{ numberFormat(sneaker.resilience / 10) }}
                ...
                <div class="flex items-start">
          <span v-if="sneaker.is_fresh"
                class="bg-green-100 text-green-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded">
            Fresh 🥬
          </span>
                </div>

                <div class="flex items-start cursor-pointer px-2.5 py-0.5 rounded"
                     :class="{'bg-green-100': isUpgradable(sneaker)}" @click="sneakerBeingViewed = sneaker">
                    <span class="mr-1">Payback in </span>
            ...
            {{ sneaker.daily_roi > 0 ? sneaker.payback_period : 'Not Profitable' }}
                    ...
                    <svg class="svg-icon ml-2 mt-1" viewBox="0 0 20 20">
                        <path fill="none"
    ...
    <DialogModal :show="sneakerBeingViewed" @close="sneakerBeingViewed = false">
        ...
        <template #content>
            <ul class="mt-3 text-sm">
                <li>
                    <span class="text-gray-600 mr-1">Liquidity Ratio</span>
                    {{ numberFormat(sneakerBeingViewed.liquidity) }}
                </li>
                <li>
                    <span class="text-gray-600 mr-1">APY</span> {{ numberFormat(sneakerBeingViewed.apy) }}%
                </li>
                <li>
                    <span class="text-gray-600">Repair {{
                        numberFormat(sneakerBeingViewed.daily_repair_gst)
                        }} GST</span>
                </li>
            </ul>
            ...
            <ul class="mt-3 text-sm">
                <li class="mt-2 flex justify-between">
                    <span class="text-gray-600 mr-1">Income</span>
                    <span class="bg-green-100 text-green-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded">
                        {{ numberFormat(sneakerBeingViewed.daily_earn_gmt) }}
                    GMT
                    </span>
                </li>
                <li class="mt-2 flex justify-between">
                    <span class="text-gray-600 mr-1">Expense</span>
                    <span class="bg-red-100 text-red-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded">
                    {{ numberFormat(sneakerBeingViewed.daily_expense_gmt) }} GMT
                    </span>
                </li>
                <li class="mt-2 flex justify-between">
                    <span class="text-gray-600 mr-1">Net Profit</span>
                    <span class="bg-green-100 text-green-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded">
                    {{ numberFormat(sneakerBeingViewed.daily_earn_gmt - sneakerBeingViewed.daily_expense_gmt) }}
                    GMT
                    </span>
                </li>
                <li>
                    <span class="text-gray-600 mr-1">Payback Period</span> {{ sneakerBeingViewed.payback_period }}
                </li>
            </ul>

            <ul class="mt-3 text-sm" v-if="sneakerBeingViewed.daily_roi < sneakerBeingViewed.daily_roi_max">
                <li>
                    <span class="text-gray-600 mr-1">Potential APY</span> {{
                    numberFormat(sneakerBeingViewed.apy_max)
                    }}%
                </li>
                <li>
                    <span class="text-gray-600 mr-1">Payback Period</span> {{ sneakerBeingViewed.payback_period_max }}
                </li>
                <li>
                    <span class="text-gray-600 mr-1">Daily Earning</span>
                    {{ numberFormat(sneakerBeingViewed.daily_earn_max_gmt) }} GMT
                </li>
                <li>
                    <span class="text-gray-600 mr-1">Daily Net Profit</span>
                    {{ numberFormat(sneakerBeingViewed.daily_earn_max_gmt - sneakerBeingViewed.daily_expense_gmt) }}
                    GMT
                </li>
            </ul>
