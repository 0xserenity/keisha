...
class HealthPoint extends Command
{
    ...
    public function handle()
    {
        ...
        $gstPrice = Price::symbol('GST') / $gmtPrice;
        ...
        $this->info(
            sprintf(
                '%d gems| %d GST for the fee| So it costs %01.2f GMT in total|||',
                $hp->getQuantity(),
                $hp->getCost() * $hp->getQuantity(),
                number_format($hp->getQuantity() * $gemPrice + $hp->getQuantity() * $hp->getCost() * $gstPrice, 2)
            )
        );

        $this->info(
            sprintf(
                'Pricing:| 1 Gem = %01.3f GMT| 1 GST = %01.2f GMT',
                $gemPrice,
